...
namespace s1_Api_Vesion1.Controllers
{
    public class ValuesController : ApiController
    {
        ...
        public IHttpActionResult History([FromBody]IDName obj)
        {
            ...
            if (obj != null)
            {
                ...
                return Json(new Result<List<History>>()
                {
                    code = 200,
                    msg = "ok",
                    data =
                    ...
                    {
                        date = s.TransferDate.ToString(),
                        from1 = s.DepartmentLocation.Department.Name,
                        from2 = s.FromAssetSN,
                        to1 = s.DepartmentLocation1.Department.Name,
                        to2 = s.ToAssetSN
                    }).ToList()
                });
    ...
    .Select(s => new IDName { id = s.ID, name = s.FirstName + s.LastName }).ToList()
        ...
        public IHttpActionResult postAsset([FromBody]AddAsset addAsset)
        {
            ...
            DateTime date1;
            if (DateTime.TryParse(addAsset.date, out date1))
            {
                db.Assets.Add(new Asset
                {
                    AssetGroupID = addAsset.assetGroup,
                    DepartmentLocationID = addAsset.DepId,
                    AssetName = addAsset.assetName,
                    AssetSN = addAsset.SN,
                    EmployeeID = addAsset.employeeId
                    ,
                    Description = addAsset.description
         ,
                    WarrantyDate = date1
                });
            }
    ...
    public class AddAsset
    {
        ...
        public long assetGroup { get; set; }

        public long DepId { get; set; }

        public long DepLocationId { get; set; }
